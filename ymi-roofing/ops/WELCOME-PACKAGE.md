# Welcome to Aurora AI Agency

**Y.M.I Roofing · Client Welcome Package**

---

Hi Ben,

Welcome aboard. This pack tells you exactly what you're getting, what I need from you, and
what happens over the next fortnight. Keep it — it doubles as your reference guide.

---

## 1. What you're getting

### Your website — **ymiroofing.com.au**
Built, tested and ready. Not a template — designed around how people actually hire a roofer
on a phone.

- **Built for mobile first.** Most people find a roofer on their phone standing in the
  driveway looking at their roof. There's a permanent "Call Ben Now" bar fixed to the bottom
  of the screen, so your number is always one tap away no matter how far they've scrolled.
- **Your real work, front and centre.** An "Our Work" gallery built from your own photos —
  the restorations, the crew on the job, the before shots. No stock photos, no fake imagery.
- **Found by the right locals.** Targeted at Melton and Melbourne's western suburbs, with the
  suburbs you actually service coded into the page so Google knows where to show you.
- **Your branding throughout.** Your logo in the header, hero and footer, plus a branded
  share card so the link looks professional when it's posted to Facebook or texted to someone.
- **Privacy policy and terms of service drafted** with Australian Consumer Law in mind —
  cooling-off rights, workmanship warranty terms and record retention. These are drafted
  documents, not legal advice; have a solicitor review them before you rely on them.

### Automation (once switched on)
- **Lead capture** — every enquiry goes to a shared Google Sheet and texts your phone instantly
- **Review machine** — tap a bookmark after a job, the customer gets a review request next day,
  with a polite nudge 3 days later if they haven't
- **Chatbot** — answers common questions on Facebook/Instagram 24/7 and routes urgent repairs
  straight to your phone

### Analytics & reviews
Google Analytics and Google review buttons are already wired into the site. They switch on the
moment you send me the account links (Section 3 below).

---

## 2. Investment

| | |
|---|---|
| **Setup (one-off)** | $700 |
| **Monthly retainer** | $350/month |
| **Invoice AURORA-0001** | $1,400 (setup + first 2 months) |

No GST is charged — Aurora isn't registered for GST.

**The retainer covers:** website hosting and maintenance, hosting and upkeep of the n8n
automation workflows, a monthly performance report, minor site updates (text, phone numbers,
prices), and chatbot monitoring and tweaks.

**Billed directly to you by the provider** (not through Aurora, and not included in the
retainer): domain renewal ~$14/yr, Twilio SMS usage ~$0.60/month, ManyChat Pro subscription
~$22/month, and any advertising you choose to run. These stay in your name so you keep
control of them.

**Cancellation and handover.** Either of us can cancel with 30 days' notice. On cancellation:
the website files, your photos and all site content are yours, and I'll hand over the source
files. The domain, Google Business Profile, Google Analytics property and any third-party
accounts (Twilio, ManyChat) are already in your name and simply stay with you. Hosting on
Aurora's account stops at the end of the notice period — I'll help you move it to your own
hosting, or hand over the files for someone else to deploy.

---

## 3. What I need from you

Everything is in the **Client Information Request** form attached to this pack. Two parts
are urgent — the rest can follow.

### 🔴 Before we can go live
1. **Your business details** — sole trader or Pty Ltd, and your ABN.
   The site currently says "Pty Ltd, ACN 695 710 055" and I need to confirm that's accurate.
   If you're a sole trader, that's completely fine and very common for trades — we just have
   to word it correctly, because describing the business as something it isn't can be
   misleading conduct under consumer law.
2. **Licence and insurance details** — ideally a Certificate of Currency from your insurer
   (they'll email it instantly on request). Right now the site says these are "available on
   request"; with the real numbers I can state them outright, which wins far more jobs.

### 🟡 As soon as you can
3. **Google Business Profile** — the single most valuable thing on this list. It's what puts
   you in Google Maps when someone searches "roof repairs near me". Bigger source of calls
   than the website itself. Guide included; I can walk you through it on the phone in 15 min.
4. **Google Analytics, Facebook, Instagram** — guides included for all three.
5. **More photos** — especially matched before + after of the *same* roof. Those convert best.
6. **2–3 past customers** happy to be quoted. I won't invent testimonials — fake reviews
   breach consumer law and get Google profiles suspended. Real ones only.

---

## 4. What happens next

| When | What |
|---|---|
| **Now** | You return the information form and pay AURORA-0001 |
| **Within 48 hrs of payment** | I correct the business details from Section A and prepare everything for launch |
| **Once Sections A + B are confirmed and I have domain access** | The site goes public and analytics switch on — not before, because until then the site would be making claims we can't back up |
| **Week 1** | Google Business Profile created and submitted for verification (you do the ID step, I do the rest) |
| **Week 2** (after go-live) | Automations live — lead alerts, review machine, chatbot |
| **Monthly** | Performance report: visitors, enquiries, calls, reviews gained |

---

## 5. How to reach me

**Email: aaron221048@gmail.com** — this is the best way to reach me and the address the
invoice comes from. Phone: `[your number]`.

For anything urgent — site down, a bug, a lead not coming through — call, don't email.

**Small changes are included** in the retainer: phone numbers, prices, service wording, adding
photos. Just send them through, no charge, no forms.

---

## 6. Straight talk

A few things worth saying up front, because they matter more than anything on the invoice:

**The website is not the thing that gets you calls.** Your Google Business Profile is. The
website's job is to convert someone who's already found you — the profile's job is to be
found. Do the profile.

**Reviews compound.** Five genuine Google reviews will outperform any amount of website
polish. Text the review link to every happy customer the day the job finishes — that's when
they'll actually do it. One a week beats ten in a day (Google flags sudden spikes).

**Photos are your best asset and you're already sitting on them.** Every job is a marketing
opportunity: one shot before, one after, same angle. Takes ten seconds. Send them through and
I'll keep the gallery growing.

**I won't fake anything on your behalf.** No invented reviews, no stock photos passed off as
your work, no credentials you don't hold. It's illegal under Australian Consumer Law, and
it's the fastest way to lose trust with the customers you're trying to win. Everything on your
site is real — that's exactly why it'll hold up.

---

Looking forward to getting this live.

**Aurora AI Agency**
ABN 45 746 317 471
